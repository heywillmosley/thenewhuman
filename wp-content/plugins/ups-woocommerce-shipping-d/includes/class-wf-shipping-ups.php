...
?>
    ...
    public function init_form_fields() {
    	...
    	$this->form_fields  = array(
            ...
            'conversion_rate'     => array(
                'title' 		  => __('Conversion Rate.', 'woo_payuindia'),
                'type' 			  => 'text',
                'default'         => '',
                'description' 	  => __('Enter the conversion amount in case you have a different currency set up comparing to the currency of origin location. This amount will be multiplied with the shipping rates. Leave it empty if no conversion required.'),
                'desc_tip' 		  => true
            ),
    ...
    private function box_shipping( $package, $params=array() ) {
		...
		foreach ( $box_packages as $key => $box_package ) {
			...
			$request  = '<Package>' . "\n";
			$request .= '	<PackagingType>' . "\n";
			$request .= '		<Code>'.$box_package->id.'</Code>' . "\n";
			$request .= '		<Description>Package/customer supplied</Description>' . "\n";
			$request .= '	</PackagingType>' . "\n";
			$request .= '	<Description>Rate</Description>' . "\n";
		...
		if( !empty($errors) ){
			...
			foreach($packages as $package){
				...
				if ((isset($params['service_code']) && $params['service_code'] == 92) || ($this->service_code == 92)) { // Surepost Less Than 1LBS
					...
					$request.= '			<Code>' . $this->weight_unit . '</Code>' . "\n";
					$request.= '		</UnitOfMeasurement>' . "\n";
					$request.= '		<Weight>' . $package_total_weight . '</Weight>' . "\n";
					$request.= '	</PackageWeight>' . "\n";
				}
				...
				if ($this->insuredvalue || $this->cod) {
					...
					if ($this->insuredvalue) {
						$request.= '		<InsuredValue>' . "\n";
						$request.= '			<CurrencyCode>' . get_woocommerce_currency() . '</CurrencyCode>' . "\n";
						$request.= '			<MonetaryValue>' . (string) ($insured_value) . '</MonetaryValue>' . "\n";
						$request.= '		</InsuredValue>' . "\n";
					}
