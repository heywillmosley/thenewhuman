...
class USIN_Pmpro_User_Activity{
	...
	protected function get_membership_activity($user_id){
		...
		foreach ($memberships as $membership) {
			$title = sprintf('%s <span class="usin-tag usin-tag-%s">%s</span>',
				$membership->name, $membership->status, $membership->status);
	...
	protected function get_memberships($user_id){
		...
		$query = $wpdb->prepare("SELECT mu.*, ml.name FROM $wpdb->pmpro_memberships_users mu".
			...
			" WHERE mu.user_id = %d ORDER BY mu.startdate DESC", $user_id);
	...
	protected function get_membership_details($membership){
		...
		$details[]=sprintf('%s: %s', __('Fee', 'usin'), $this->get_price($membership));
		$details[]=sprintf('%s: %s', __('Start date', 'usin'), USIN_Helper::format_date($membership->startdate));
		if(!empty($membership->enddate) && $membership->enddate != '0000-00-00 00:00:00'){
			$details[]=sprintf('%s: %s', __('End date', 'usin'), USIN_Helper::format_date($membership->enddate));
		}
