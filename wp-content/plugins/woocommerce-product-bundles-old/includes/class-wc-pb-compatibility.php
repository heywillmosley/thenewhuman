...
class WC_PB_Compatibility {
	...
	public function __construct() {
		...
		if ( class_exists( 'WC_Composite_Products' ) ) {
			...
			// Show bundle type products using the bundle-product.php composited product template -- CP v3.2
			add_action( 'woocommerce_composite_show_composited_product_bundle', array( $this, 'composite_show_product_bundle' ), 10, 3 );
		...
		add_filter( 'woocommerce_order_amount_item_subtotal', array( $this, 'order_amount_bundle_subtotal' ), 10, 5 );
	 ...
	 * @return int
	 */
	public function composited_bundle_in_cart_item_quantity( $quantity, $cart_item_key ) {
	 ...
	 * @return int
	 */
	public function composited_bundle_checkout_item_quantity( $quantity, $cart_item_values, $cart_item_key = false ) {
	 ...
	 * @return array
	 */
	public function order_add_bundled_meta( $items, $order ) {
				...
				if ( isset( $item[ 'bundled_items' ] ) && isset( $item[ 'bundle_cart_key' ] ) && isset( $item[ 'per_product_shipping' ] ) && $item[ 'per_product_shipping' ] === 'no' ) {
					...
					foreach ( $items as $child_item ) {
						...
						if ( isset( $child_item[ 'bundled_by' ] ) && $child_item[ 'bundled_by' ] === $bundle_key ) {
							...
							$meta_values[] = $woocommerce_bundles->helpers->format_product_title( $title, $sku, $meta, true );
						}
					}
	 ...
	 * @return double
	 */
	public function order_amount_bundle_total_or_subtotal( $price, $order, $item, $inc_tax, $round, $total_or_subtotal ) {
			...
			if ( isset( $item[ 'bundled_items' ] ) && isset( $item[ 'bundle_cart_key' ] ) && isset( $item[ 'bundled_shipping' ] ) && $item[ 'bundled_shipping' ] === 'yes' ) {
				...
				foreach ( $order->get_items( 'line_item' ) as $order_item ) {
					...
					if ( isset( $order_item[ 'bundled_by' ] ) && $order_item[ 'bundled_by' ] === $bundle_key ) {
						if ( $total_or_subtotal === 'total' ) {
							$bundle_value += $order->get_line_total( $order_item, $inc_tax, $round ) / $bundle_qty;
						} else {
							$bundle_value += $order->get_line_subtotal( $order_item, $inc_tax, $round ) / $bundle_qty;
						}
					}
				}
				...
				if ( $parent && isset( $parent[ 'per_product_shipping' ] ) && $parent[ 'per_product_shipping' ] === 'yes' && isset( $parent[ 'per_product_pricing' ] ) && $parent[ 'per_product_pricing' ] === 'no' && isset( $parent[ 'bundle_cart_key' ] ) ) {
					...
					if ( $total_or_subtotal === 'total' ) {
						...
						$bundle_value = $order->get_line_subtotal( $parent, $inc_tax, $round );
					}
					...
					foreach ( $order->get_items( 'line_item' ) as $child_item ) {
						...
						if ( isset( $child_item[ 'bundled_by' ] ) && $child_item[ 'bundled_by' ] === $bundle_key ) {
							...
							if ( $total_or_subtotal === 'total' ) {
								...
								$bundle_value += $order->get_line_subtotal( $child_item, $inc_tax, $round );
							}
	 ...
	 * Shipstation compatibility
	 *
	 * @see order_amount_bundle_total_or_subtotal
	 */
	public function order_amount_bundle_total( $price, $order, $item, $inc_tax, $round ) {
		return $this->order_amount_bundle_total_or_subtotal( $price, $order, $item, $inc_tax, $round, 'total' );
	}
	 ...
	 * @see order_amount_bundle_total_or_subtotal
	 */
	public function order_amount_bundle_subtotal( $price, $order, $item, $inc_tax, $round ) {
		return $this->order_amount_bundle_total_or_subtotal( $price, $order, $item, $inc_tax, $round, 'subtotal' );
	}
	 ...
	 * @return double
	 */
	public function cost_of_goods_checkout_order_bundled_item_cost( $cost, $values, $cart_item_key ) {
	 ...
	 * @return double
	 */
	public function cost_of_goods_set_order_item_bundled_item_cost( $cost, $item, $order ) {
	 ...
	 * @return void
	 */
	public function opc_single_add_to_cart_bundle( $opc_post_id ) {
	 ...
	 * @return bool
	 */
	public function opc_disallow_bundled_cart_item_modification( $allow, $cart_item, $cart_item_key, $opc_id ) {
	 ...
	 * @return WC_Product
	 */
	public function addons_price_for_display_product( $product ) {
	 ...
	 * @return boolean
	 */
	public function bundled_order_item_is_child_of_composite( $is_child, $order_item, $composite_item, $order ) {
	 ...
	 * @return boolean
	 */
	public function bundled_cart_item_is_child_of_composite( $is_child, $cart_item_key, $cart_item_data, $composite_key, $composite_data ) {
	 ...
	 * @return void
	 */
	public function composite_show_product_bundle( $product, $component_id, $composite ) {
	 ...
	 * @return array
	 */
	public function composite_bundle_cart_item_stamp( $composited_item_identifier, $composited_item_id ) {
	 ...
	 * @return boolean
	 */
	public function disallow_bundled_item_subs( $passed, $bundle ) {
	 ...
	 * @return boolean
	 */
	public function composite_validate_bundle_data( $result, $composite_id, $component_id, $bundle_id, $quantity, $cart_item_data ) {
	 ...
	 */
	public function associated_bundle_stock( $items, $composite_id, $component_id, $bundled_product_id, $quantity ) {
	 ...
	 * @return string
	 */
	public function bundle_field_prefix( $prefix, $product_id ) {
	 ...
	 * @return array
	 */
	public function remove_bundled_pre_orders_cart_item_meta( $pre_order_meta, $cart_item_data ) {
	 ...
	 * @return array
	 */
	public function remove_bundled_pre_orders_order_item_meta( $pre_order_meta, $order_item, $order ) {
	 ...
	 * @return void
	 */
	public function points_rewards_remove_price_html_messages( $bundled_item ) {
	 ...
	 * @return void
	 */
	public function points_rewards_restore_price_html_messages( $bundled_item ) {
	 ...
	 * @return void
	 */
	public function return_empty_message( $message ) {
		return false;
	}
	 ...
	 * @return string
	 */
	public function points_rewards_bundle_message( $message, $points_n_rewards ) {
	 ...
	 * @return int
	 */
	public function points_earned_for_bundled_order_item( $points, $product, $item_key, $item, $order ) {
	 ...
	 * @return int
	 */
	public function points_earned_for_bundled_cart_item( $points, $cart_item_key, $cart_item_values ) {
	 ...
	 * @return void
	 */
	public function after_bundled_add_to_cart( $product_id, $quantity, $variation_id, $variations, $bundled_item_cart_data ) {
	 ...
	 * @return void
	 */
	public function before_bundled_add_to_cart( $product_id, $quantity, $variation_id, $variations, $bundled_item_cart_data ) {
	 ...
	 * @return array
	 */
	public function get_bundled_cart_item_data_from_parent( $bundled_item_cart_data, $cart_item_data ) {
	 ...
	 * @return array
	 */
	public function bundled_item_addons_stamp( $bundled_item_stamp, $bundled_item_id ) {
	 ...
	 * @return array
	 */
	public function bundled_item_nyp_stamp( $bundled_item_stamp, $bundled_item_id ) {
	 ...
	 * @return bool
	 */
	public function validate_bundled_item_nyp_and_addons( $add, $bundle, $bundled_item, $quantity, $variation_id ) {
	 ...
	 * @return void
	 */
	public function addons_support( $product_id, $item ) {
	 ...
	 */
	public function addons_cart_prefix( $prefix, $product_id ) {
	 ...
	 * @return void
	 */
	public function nyp_price_input_support( $product_id, $item ) {
	 ...
	 * @return string                   a unique prefix
	 */
	public function nyp_cart_prefix( $prefix, $product_id ) {
