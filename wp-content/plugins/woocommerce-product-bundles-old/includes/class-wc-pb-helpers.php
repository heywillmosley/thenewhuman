...
class WC_PB_Helpers {
	 ...
	 * @return string
	 */
	public function format_product_title( $title, $sku = '', $meta = '', $paren_meta = false ) {

		if ( $sku && $meta ) {
			if ( $paren_meta ) {
				$title = sprintf( _x( '%1$s &ndash; %2$s (%3$s)', 'product title followed by sku and meta in parenthesis', 'woocommerce-product-bundles' ), $sku, $title, $meta );
			} else {
				$title = sprintf( _x( '%1$s &ndash; %2$s &mdash; %3$s', 'product title followed by sku and meta', 'woocommerce-product-bundles' ), $sku, $title, $meta );
			}
		} elseif ( $sku ) {
			$title = sprintf( _x( '%1$s &ndash; %2$s', 'product title followed by sku', 'woocommerce-product-bundles' ), $sku, $title );
		} elseif ( $meta ) {
			if ( $paren_meta ) {
