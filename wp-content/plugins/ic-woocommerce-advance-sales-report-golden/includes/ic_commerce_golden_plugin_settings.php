...
if ( ! class_exists( 'IC_Commerce_Golden_Settings' ) ) {
	class IC_Commerce_Golden_Settings extends IC_Commerce_Golden_Functions{
		...
		function save_settings(){			
			...
			if(isset($_POST[$option]) and (isset($_POST['option_page']) and $_POST['option_page'] == $option) and  isset($_POST['option_page'])){
				...
				if(!isset($post['hide_order_status'])){
					$post['hide_order_status'] = '';
				}
				...
				//New for Email Schedule Dashbord report page 20150205				
				...
				if(strlen($ga_account)>0 || strlen($ga_password)>0){
					include_once("ic_commerce_golden_gapi.class.php");
				...
				$post = apply_filters("ic_commerce_golden_settting_values", $post, $this);
				
				//New Change ID 20140918
				if($o){
					update_option($option,$post);
				}else{delete_option($option);
					add_option($option,$post);
				}
		...
		public function init_settings() {
			...
			*/
			...
			//$email_from_name	= $blog_title;//Modified 20150217
			//$email_from_email	= $email_send_to;//Modified 20150217
			//$email_subject		= $blog_title;//Modified 20150217
			...
			$new_shop_order_status["all"] = __("All Status",'icwoocommerce_textdomains');
			...
			$product_statuses = array(
					"-1"			=> __("All Status",	'icwoocommerce_textdomains'),
					"auto-draft" 	=> __("Auto Draft",	'icwoocommerce_textdomains'),
					"draft" 		=> __("Drafts",		'icwoocommerce_textdomains'),
					"publish"		=> __("Published",	'icwoocommerce_textdomains'),
					"trash" 		=> __("Trash",		'icwoocommerce_textdomains')
			);
			...
			//section			
			add_settings_section('product_image_sections',			__(	'Product Image Section:', 'icwoocommerce_textdomains'),		array( &$this, 'section_options_callback' ),$option);
			
			add_settings_field('show_product_image',			__( 'Show Product Image:', 'icwoocommerce_textdomains'),		array( &$this,'checkbox_element_callback' ),$option, 'product_image_sections', array('menu'=> $option,	'value'=>'1',	'label_for'=>'show_product_image','id'=> 'show_product_image','default'=>0,'description' => __("Product Image will be displayed on Variaiton Report page.")));
			...
			add_settings_field($order_status_field,				__( 'Shop Order Status:', 'icwoocommerce_textdomains'),					array( &$this, 'select_element_callback' ), 	$option, 'dashboard_setting', array('menu'=> $option,	'size'=>8,	'class'=>'numberonly', 'maxlength'=>'5',	'label_for'=>$order_status_field,				'id'=> $order_status_field,					'default'=>$detault_stauts_id,  'multiple'=>'multiple',	'options'=> $new_shop_order_status, 'description' => __("Ctrl + click to multiselect. <br /> Selected status will be used for calculating sales amount.",'icwoocommerce_textdomains')));
			add_settings_field('product_status',				__( 'Product Status:', 'icwoocommerce_textdomains'),					array( &$this, 'select_element_callback' ), 	$option, 'dashboard_setting', array('menu'=> $option,	'size'=>5,	'class'=>'numberonly', 'maxlength'=>'5',	'label_for'=>'product_status',				'id'=> 'product_status',					'default'=>'publish',  'multiple'=>'multiple',	'options'=> $product_statuses, 'description' => __("Ctrl + click to multiselect. <br /> Selecting \"Select All\" will displayed all products including Auto Draft, Draft, Published, Trash and Deleted.",'icwoocommerce_textdomains')));
			...
			add_settings_field('transaction_products',			__( 'Transaction Products:', 'icwoocommerce_textdomains' ),				array( &$this,'checkbox_element_callback' ),	$option, 'dashboard_setting', array('menu'=> $option,	'value'=>'yes',	'label_for'=>'transaction_products','id'=> 'transaction_products','default'=>0,'description' => __("All product filter dropdown display product list from transaction products.",'icwoocommerce_textdomains')));
			
			
			
			
			add_settings_section('cross_tab',				__(	'Cross Tab Reports:','icwoocommerce_textdomains' ),				array( &$this, 'section_options_callback' ),	$option);
			add_settings_field('cross_tab_start_date',		__( 'Cross Tab Start Month:', 'icwoocommerce_textdomains' ),		array( &$this, 'text_element_callback' ), 		$option, 'cross_tab', 		array('menu'=> $option,	'size'=>15,	'class'=>'numberonly', 'maxlength'=>'10',	'label_for'=>'cross_tab_start_date',	'id'=> 'cross_tab_start_date',		'default'=>'',		'readonly'=> true));
			add_settings_field('cross_tab_end_date',		__( 'Cross Tab End Month:', 'icwoocommerce_textdomains' ),			array( &$this, 'text_element_callback' ), 		$option, 'cross_tab', 		array('menu'=> $option,	'size'=>15,	'class'=>'numberonly', 'maxlength'=>'10',	'label_for'=>'cross_tab_end_date',		'id'=> 'cross_tab_end_date',		'default'=>'',		'readonly'=> true));
			...
			if(!$email_subject || strlen($email_subject) <= 0){
				...
				//$email_subject = $email_subject. "#COMMA_SEPARATED_REPORTS#";
			...
			*/
			...
			add_settings_field('email_schedule',	__( 'Email Schedule:', 'icwoocommerce_textdomains' ),	array( &$this, 'select_element_callback' ), 	$option, 'email_reports', array('menu'=> $option,	'label_for'=>'email_schedule',		'id'=> 'email_schedule','default'=>'daily','options'=>array(
			...
			),'label_none'=>__('Unschedule Event','icwoocommerce_textdomains')));
			...
			do_action('ic_commerce_golden_settting_field_after_email_report',$this, $option);
			/////////////////////
			...
			add_settings_field('pdf_invoice_logo', 				__( 'Logo Image:', 'icwoocommerce_textdomains' ),						array( &$this, 'choose_image_callback' ), 		$option, 'pdf_invoice_setting', array('menu'=> $option,	'size'=>40,	'class'=>'numberonly', 'maxlength'=>'500', 	'label_for'=>'pdf_invoice_logo', 	'id'=> 'pdf_invoice_logo',			'default'=>'', 	'choose_id'=>'upload_pdf_invoice_logo_button'));
			...
			add_settings_field('ga_profile_id',			__( 'Profile ID:', 'icwoocommerce_textdomains' ),					array( &$this, 'text_element_callback' ), 		$option, 'google_analytics', array('menu'=> $option,	'size'=>25,	'class'=>'emailcharacters', 'maxlength'=>'250',	'label_for'=>'ga_profile_id',	'id'=> 'ga_profile_id','autocomplete' => "off"));
		...
		function create_button_element_callback( $args ) {
			...
			foreach($buttons as $btn => $bt){
				...
				$type 	= $bt['type'];
				$value 	= $bt['value'];
				$html .= sprintf( '<input type="%1$s" id="%2$s" name="%3$s[%2$s]" value="%4$s" class="%1$s onformprocess button" />', $type, $id, $menu, $value);
			}
