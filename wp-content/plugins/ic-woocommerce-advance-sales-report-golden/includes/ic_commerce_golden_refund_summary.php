...
if(!class_exists('IC_Commerce_Golden_Refund_Summary')){
	
	class IC_Commerce_Golden_Refund_Summary  extends IC_Commerce_Golden_Functions{
		...
		function ic_commerce_custom_all_refund_summary_query($rows = '', $type = '', $columns = '', $report_name = '', $that = ''){
			...
			if(!isset($this->items_query)){
				...
				if($group_by == 'order_id' || $group_by == 'refund_id'|| $group_by == 'refunded'){
					$sql = "SELECT ";				
					$sql .= " SUM(refund_amount.meta_value)  	AS total_refund_amount";				
					$sql .= " ,SUM(refund_amount.meta_value)  	AS total_amount";
