...
if ( ! class_exists( 'IC_Commerce_Golden_Functions' ) ) {
	class IC_Commerce_Golden_Functions{
		...
		function get_product_data($product_type = 'all', $transaction_products = 'no'){
				...
				$publish_order			= 'no';
				$transaction_products 	= $this->get_setting('transaction_products',$this->constants['plugin_options'], $transaction_products);
				$product_status 		= $this->get_setting('product_status',$this->constants['plugin_options'], array('publish'));
				...
				if($transaction_products == "yes"){
					...
					if($publish_order == "yes")	$sql .= " LEFT JOIN {$wpdb->prefix}posts AS posts ON posts.ID = woocommerce_order_items.order_item_id";
					...
					if(count($product_status)>0){
						$product_status2		= (count($product_status) == 1 and $product_status[0] == '-1') ? '-1' : $product_status[0];
						
						if($product_status2 != '-1'){
					...
					if(count($product_status)>0){
						...
						if($product_status2 != '-1'){
							$in_product_status = implode("','",$product_status);
							$sql .= " AND posts.post_status IN ('{$in_product_status}')";
						}
					}
		...
		function get_post_type_products($product_status = array()){
		...
		function get_custom_field_data($order_item = NULL, $meta_key = NULL, $default = NULL ){
			...
			return $ic_commerce_golden_custom_fields->get_custom_field_data($order_item,$meta_key,$default);
		}
		...
		function export_to_pdf($export_rows = array(),$output){
			if(count($export_rows)>0){
				...
				//require_once("ic_commerce_golden_dompdf_config.inc.php");
				//$dompdf = new DOMPDF();	
		...
		function get_set_status_ids(){
			...
			function get_grid_columns(){
				if($this->grid_column){
					...
					include_once('ic_commerce_golden_columns.php');
					$grid_column 		= new IC_Commerce_Golden_Columns($this->constants);
			...
			function get_grid_object(){
				include_once("ic_commerce_golden_grid_data.php");				
				return $object = new IC_Commerce_Golden_Grid_Data($this->constants);
			}
		...
		/*20150331*/
		function get_refund_amount($post_parent_id=NULL)
		{
			global $wpdb;
			...
			if ($post_parent_id)
				$sql .= "AND posts.post_parent = {$post_parent_id}";
		...
		function get_variaiton_attributes_columner_separated($variation_by = 'variation_id', $variation_ids = '', $order_item_ids = ''){			
					global $wpdb;
					$variation_order_item_ids = $new_item_attr_order_item_ids = array();
			...
			function get_product_variation_attributes($all_columns = "no"){
				...
				$sql = "SELECT postmeta_product_addons.meta_value AS product_attributes, posts.ID AS product_id FROM {$wpdb->prefix}posts AS posts";
				$sql .= " LEFT JOIN {$wpdb->prefix}postmeta AS postmeta_product_addons ON postmeta_product_addons.post_id = posts.ID";
				$sql .= " WHERE post_type in ('product')";
				$sql .= " AND postmeta_product_addons.meta_key IN ('_product_attributes') ";
				$sql .= " AND postmeta_product_addons.meta_value NOT IN ('a:0:{}','') ";
				
				//$parent_attributes       = get_post_meta( 1115, '_product_attributes', true );
				//$this->print_array($parent_attributes);
				...
				if(count($product_addon_objects)>0){					
					foreach($product_addon_objects as $key => $value){
						...
						$product_addon_lists     = get_post_meta($product_id, '_product_attributes', true );
						
						foreach($product_addon_lists as $key2 => $value2){
							$product_addon_master[] = $key2;
							$variations[$key2] 		= $value2['name'];
						}
						
						/*if(!empty($product_attributes)){
							$product_addon_lists = maybe_unserialize($product_attributes);
							foreach($product_addon_lists as $key2 => $value2){
								//$product_addon_master[] = $key2;
								//$this->print_array($key2);
							}
						}*/
						//$this->print_array($product_addon_lists);
					}
				...
				if($product_addon_master_key){
					...
					if(count($items_objects)>0){					
						foreach($items_objects as $key => $value){
							$product_addon_master[] = $key2;
							$product_attirbute_columns[$value->attribute_key_label] = isset($variations[$value->attribute_key]) ? $variations[$value->attribute_key] : $value->attribute_key;
						}
					}
			...
			function _____get_product_variation_attributes($all_columns = "no"){
				global $wpdb;	
			...
			function get_wccs_filter_colummns(){
				$wccs_column_filter = array();
				$wccs_settings = get_option('wccs_settings', array());				
				$wccs_columns_settings = isset($wccs_settings['buttons']) ? $wccs_settings['buttons'] : array();				
				foreach($wccs_columns_settings as $column_key => $column_label){
					$cow = isset($column_label['cow']) ? $column_label['cow'] : '';
					$wccs_columns[$cow] = $column_label['label'];
				}
				
				return $wccs_columns;
			}
			
			function get_wccs_filter_filds(){
				$wccs_column_filter = array();
				$wccs_settings = get_option('wccs_settings', array());				
				$wccs_columns_settings = isset($wccs_settings['buttons']) ? $wccs_settings['buttons'] : array();				
				foreach($wccs_columns_settings as $column_key => $column_label){
					...
					$option_a = isset($column_label['option_a']) ? $column_label['option_a'] : '';
					$option_b = isset($column_label['option_b']) ? $column_label['option_b'] : '';
					
					if(!empty($option_a) and !empty($option_b)){
						
						$wccs_column_filter[$cow]['label'] 	= $column_label['label'];
						$wccs_column_filter[$cow]['id']		= $cow;
						$wccs_column_filter[$cow]['data'][$option_a] = $option_a;
						$wccs_column_filter[$cow]['data'][$option_b] = $option_b;
					}
					
				}
				
				return $wccs_column_filter;
			}
			
			function get_wccs_form_filter_fields(){
				$filter_filds = $this->get_wccs_filter_filds();
				//$this->print_array($filter_filds);
				$output = "";
				if(count($filter_filds)>0){
					$vi = 0;
					$output .= '<div class="form-group dynamic_fields">';
					foreach($filter_filds as $key => $values):
						
						//echo $key;
						//$this->print_array($values);
						$label  = isset($values['label']) 	? $values['label'] : array();
						$id 	= isset($values['id']) 		? $values['id'] : array();
						...
						if(count($data)>0){
							...
							$detault =  '-1';
							$output .= '<div class="FormRow'.($vi%2 ? ' SecondRow' : ' FirstRow').' wccs_dropdown_'.$id.'">';
							$output .= '<div class="label-text"><label for="wccs_dropdown_'. $id.'">'.$label.':</label></div>';
							$output .= '<div class="input-text">';
								$attribute_values = $values;
								$output .= $this->create_dropdown($data,"wccs_dropdown[$key][]","wccs_dropdown_{$id}",'Select One',"wccs_dropdowns",'-1', 'array', false, 0, $detault, false);
							$output .= '</div>';
							$output .= '</div>';
							$vi++;
						}
						...
						$output .= '<div class="label-text"><label for="variations_'. $id.'">'.$label.':</label></div>';
						$output .= '<div class="input-text">';
							$attribute_values = $values;
							$output .= $this->create_dropdown($attribute_values,"new_variations_value[$key][]","variation_dropdown_{$id}",'Select All',"variation_dropdowns",'-1', 'array', true, 5, $detault, false);
						$output .= '</div>';
						$output .= '</div>';
						$vi++;
						*/
