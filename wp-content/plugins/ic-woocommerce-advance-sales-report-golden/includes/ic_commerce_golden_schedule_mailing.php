...
if(!class_exists('IC_Commerce_Golden_Schedule_Mailing')){
	class IC_Commerce_Golden_Schedule_Mailing{
		...
		public function __construct($file,$plugin_key = 'icpro') {
			...
			add_action('admin_init', array($this, 'schedule_mailing_sales_status_force'),100);
		...
		function schedule_mailing_sales_status(){
			...
			$ic_commerce 									= new IC_Commerce_Golden_Schedule_Mailing_Sales_Status( $this->constants['plugin_file'], $ic_commerce_constant);
			return $ic_commerce;
		}
		...
		function cron_schedules($schedules){
			...
			$schedules['minute']		= isset($schedules['minute']) 		? $schedules['minute'] 		:	array('interval'=>	MINUTE_IN_SECONDS,		'display'=> __('Once Minute',	'icwoocommerce_textdomains'));//For testing
			
			$schedules['hourly']		= isset($schedules['hourly']) 		? $schedules['hourly'] 		:	array('interval'=>	HOUR_IN_SECONDS,		'display'=> __('Once Hourly',	'icwoocommerce_textdomains'));	
			$schedules['daily']			= isset($schedules['daily']) 		? $schedules['daily'] 		:	array('interval'=>	DAY_IN_SECONDS,			'display'=> __('Once Daily',	'icwoocommerce_textdomains'));
			$schedules['weekly'] 		= isset($schedules['weekly']) 		? $schedules['weekly'] 		:	array('interval'=>	WEEK_IN_SECONDS,		'display'=> __('Once Weekly',	'icwoocommerce_textdomains'));
			
			$schedules['twicehourly']	= isset($schedules['twicehourly']) 	? $schedules['twicehourly']	:	array('interval'=>	HOUR_IN_SECONDS/2,		'display'=> __('Twice Hourly',	'icwoocommerce_textdomains'));
			$schedules['twicedaily']	= isset($schedules['twicedaily']) 	? $schedules['twicedaily'] 	:	array('interval'=>	DAY_IN_SECONDS/2,		'display'=> __('Twice Daily',	'icwoocommerce_textdomains'));
			$schedules['twiceweekly']	= isset($schedules['twiceweekly']) 	? $schedules['twiceweekly'] :	array('interval'=>	WEEK_IN_SECONDS/2,		'display'=> __('Twice Weekly',	'icwoocommerce_textdomains'));
