...
if (!class_exists('IC_Commerce_Golden_Ajax_Graph')) {
    class IC_Commerce_Golden_Ajax_Graph extends IC_Commerce_Golden_Functions{
        ...
        public function __construct($constants)
        {
            ...
            switch ($do_action) {
                ...
                case "thirty_days_visit":
                    ...
                    break;
                case "ga_summary":
                    echo $return = $this->get_ga_summary($shop_order_status, $hide_order_status, $start_date, $end_date);
                    return;
                    break;
                default:
		...
		function get_part_order_refunded_amount_by_month($shop_order_status, $hide_order_status, $start_date, $end_date, $month_count = ''){
            ...
            $sql = " SELECT SUM(postmeta.meta_value) 	as total_amount,  DATE_FORMAT(posts.post_date,'%Y-%m') AS 'Month'";
        ...
        function bar_chart_sales_by_month($shop_order_status, $hide_order_status, $start_date, $end_date)
        {
			...
			//$start_date = date("Y-m-01",strtotime('-365 day', $end_date_strtotime));			
			$start_date = date('Y-m-01', strtotime("-11 month", $end_date_strtotime));
			
            $sql = " SELECT ";
            $sql .= "  MONTHNAME(posts.post_date) 				AS 'Month'";
            $sql .= ", SUM(meta_value) 							AS 'TotalAmount'";			
			$sql .= ", DATE_FORMAT(posts.post_date,'%Y-%m') 	AS month_key";
            $sql .= "  FROM {$wpdb->prefix}posts 				AS posts";
			
            $sql .= "  LEFT JOIN  {$wpdb->prefix}postmeta as postmeta ON posts.ID=postmeta.post_id";
			...
			/*
            	$sql .= " DATE(posts.post_date) >= DATE_SUB(now(), interval 11 month) AND NOW()";
            */
		
			$sql .= "  AND post_type='shop_order'";
           ...
           if($this->constants['post_order_status_found'] == 0 ){
			...
			$part_refunds = $this->get_part_order_refunded_amount_by_month($shop_order_status, $hide_order_status, $start_date, $end_date, '');
			
            foreach ($order_items as $key => $order_item) {
                $Month 				= $order_item->Month;
                $Amount            	= $order_item->TotalAmount;
				$month_key          = $order_item->month_key;
				$part_refund		= isset($part_refunds[$month_key]) ? $part_refunds[$month_key] : 0;
                $dataArray[$Month] 	= $Amount;
                $datamonth[] 		= $Month;
            }
        ...
        function get_last_week_order($shop_order_status, $hide_order_status, $start_date, $end_date)
        {
            ...
            $start_date = date("Y-m-d",strtotime('-'.($per_week*7).' day', strtotime($end_date)));
        ...
        function thirty_days_visit()
        {
            ...
            include_once('ic_commerce_golden_ga_data.php');
            $ga_data = new IC_Commerce_Golden_GA_Data($this->constants);
