...
class WP_E_Invite extends WP_E_Model {
    ...
    public static function get_invite_url($invite_hash, $invite_checksum) {
        return esc_url(add_query_arg(array('invite' => $invite_hash, 'csum' => $invite_checksum), get_permalink(WP_E_Sig()->setting->get_default_page())));
    }

    public static function get_preview_url($document_id, $hash = false) {
        if ($hash) {
            return esc_url(add_query_arg(array('esigpreview' => 1, 'document_id' => $document_id, 'hash' => $hash), get_permalink(WP_E_Sig()->setting->get_default_page())));
        } else {
            return esc_url(add_query_arg(array('esigpreview' => 1, 'document_id' => $document_id), get_permalink(WP_E_Sig()->setting->get_default_page())));
        }
    }

    public static function get_signed_doc_url($check_sum, $invite_hash) {
        return esc_url(add_query_arg(array('invite' => $invite_hash, 'csum' => $check_sum), get_permalink(WP_E_Sig()->setting->get_default_page())));
    }
