...
function _getBMPimage($data, $file) {
	$info = array();
		...
		if ($this->mpdf->restrictColorSpace==1 || $this->mpdf->PDFX || $this->mpdf->restrictColorSpace==3) {
			...
			return array('error' =>sprintf(__("BMP Image cannot be converted to suitable colour space %s - (Image replaced by 'no-image'.)","esig-pdf"),$file)); 
		}
		...
		switch ($biBitCount){
		  ...
		  default:
			return array('error' => __('Error parsing BMP image - Unsupported image biBitCount','esig-pdf')); 
		}
