    ...
    j.ajax({
        ...
        private function text_to_html($placeholder, $name, $value = '', $is_required = false, $signed = false, $verifysigner = 'undefined', $size = 'undefined', $displaytype) {
            ...
            if ($signed) {
                ...
                return '<input   placeholder="' . _wp_specialchars($placeholder) .
        ...
        private function textarea_to_html($placeholder, $name, $value = '', $is_required = false, $signed = false, $verifysigner = 'undefined', $size,$displaytype) {
            ...
            if ($signed) {
                ...
                if($displaytype == 'border')
                {
                    ...
                    return '<span ' . $verify . ' class="sif-textarea-border">' . stripslashes($value) . '</span>';
                }
                elseif($displaytype == 'underline'){
                ...
                elseif($displaytype == 'plaintext'){
                   ...
                   return '<span>' . stripslashes($value) . '</span>'; 
                    ...
                    return '<span  class="sif-textarea-border">' . stripslashes($value) . '</span>';
                }
                //return '<textarea rows="'. $rows .'" cols="'. $cols .'" style="border:1px solid #ccc;text-align:left;padding:5px;" />'. $value .' </textarea>';
               // return '<b><u>' . stripslashes($value) . '</u></b>';
                //return '<div class="esig-sif-pdf" width="'. $width .'%" '. $verify .'><span class="esig-sif-textfield signed">'. htmlspecialchars(stripslashes($value),ENT_QUOTES) .'</span></div>';
            } else {
                ...
                $html = '<textarea ' . $verify . ' name="' . $name . '"  placeholder="' . $placeholder . '" rows="' . $rows . '" cols="' . $cols . '" ' . $required . '></textarea>';
                return $html;
        ...
        private function datepicker_to_html($value = '', $name, $signed = false, $verifysigner = 'undefined', $is_required = false, $label = false,$displaytype=null) {
            ...
            if ($signed) {
                ...
                //return '<label>'. $label .' <input value="'. $value .'" readonly  style="border:1px solid #ccc;text-align:center;padding:1px;min-width:120px;width:'. $width .'%" /></label>';
			...
			j( "#' . $name . '" ).datepicker( "option", "dateFormat", "' . $this->date_format_php_to_js($date_format) . '" ).valid();
                ...
                $html .='<label> ' . $label . '  <input class="esig-sif-datepicker" ' . $verify . ' placeholder="Select Date" id="' . $name . '" type="text"  name="' . $name . '" value="" ' . $required . '  /></label> ';
        ...
        public function render_shortcode_todaydate($atts) {
            ...
            if (isset($document_id) && intval($document_id)) {
                ...
                // Already signed
                if ($esig->user->hasSignedDocument($recipient->user_id, $doc_id)) {
