...
class WC_Memberships_Members_Area {
	 ...
	 * @return string
	 */
	private function get_members_area_memberships_endpoint_url() {

		$user_memberships = wc_memberships_get_user_memberships();

		if ( 1 === count( $user_memberships ) && ( $user_membership = $user_memberships[0] ) ) {
	...
	public function add_account_members_area_menu_item( $items ) {
		...
		if ( $this->is_members_area_section() ) {
			...
			// display new endpoint if there is at least 1 membership plan defined
			if ( wc_memberships()->get_plans_instance()->get_membership_plans_count() > 0 ) {

				$endpoint_title = esc_html( $this->get_members_area_memberships_endpoint_title() );
				$endpoint_url   = $this->get_members_area_memberships_endpoint_url();

				if ( array_key_exists( 'orders', $items ) ) {
					$items = SV_WC_Helper::array_insert_after( $items, 'orders', array( $endpoint_url => $endpoint_title ) );
				} else {
					$items[ $endpoint_url ] = $endpoint_title;
				}
			}
		}
	...
	public function adjust_account_members_area_menu_item_classes( $classes, $endpoint ) {
		...
		if ( ! $this->is_members_area_section() ) {
			...
			$members_area_endpoint = $this->get_members_area_memberships_endpoint_url();
	...
	public function get_members_area_navigation_items( $membership_plan ) {
		...
		foreach ( $available_sections as $section_id => $section_name ) {
			if ( in_array( $section_id, $plan_sections, true ) ) {
				 ...
				 */
				$section_name = apply_filters( "wc_memberships_members_area_{$section_id}_title", $section_name, $this->get_members_area_user_membership() );
