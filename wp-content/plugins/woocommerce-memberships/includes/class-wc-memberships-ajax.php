...
class WC_Memberships_AJAX {
	...
	public function create_user_for_membership() {
		...
		$user       = wc_create_new_customer( $email, $username, $password );

		if ( ! is_numeric( $user ) ) {

			$error_message  = '';
			$error_messages = $user instanceof WP_Error ? $user->get_error_messages() : null;
	...
	public function transfer_user_membership() {
		...
		if ( isset( $_POST['prev_user'], $_POST['new_user'] ) && ! empty( $_POST['membership'] ) ) {
			...
			if ( empty( $new_user ) ) {
				wp_send_json_error( __( 'Please select an user to transfer the membership to.', 'woocommerce-memberships' ) );
			} elseif ( $prev_user === $new_user ) {
				wp_send_json_error( __( 'The user you have selected to transfer the membership to is the same user owning the membership to be transferred. Please select a different user.', 'woocommerce-memberships' ) );
			} elseif ( $user_membership && (int) $user_membership->get_user_id() === $prev_user && $user_membership->transfer_ownership( $new_user ) ) {
