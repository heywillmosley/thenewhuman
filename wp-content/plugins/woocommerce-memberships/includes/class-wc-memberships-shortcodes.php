			...
			if ( $post ) {
				...
				if ( in_array( get_post_type( $post_id ), array( 'product', 'product_variation' ), true ) ) {
					...
					if ( ! current_user_can( 'wc_memberships_view_restricted_product', $post->ID ) ) {
						$output .= WC_Memberships_User_Messages::get_message_html( 'product_viewing_restricted' );
					} elseif ( ! current_user_can( 'wc_memberships_view_delayed_post_content', $post->ID ) ) {
						$output .= WC_Memberships_User_Messages::get_message_html( 'product_access_delayed' );
					}

				} else {

					if ( ! current_user_can( 'wc_memberships_view_restricted_post_content', $post->ID ) ) {
						$output .= WC_Memberships_User_Messages::get_message_html( 'content_restricted' );
					} elseif ( ! current_user_can( 'wc_memberships_view_delayed_post_content', $post->ID ) ) {
						$output .= WC_Memberships_User_Messages::get_message_html( 'content_delayed' );
					}
				}
			}
		}
