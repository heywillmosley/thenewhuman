...
class WC_Memberships_Rules {
	...
	public function is_valid_rule_content_type_name( $name, $rule_type ) {
		...
		$is_products = in_array( $name, array( 'product', 'product_cat' ), true );
							...
							case 'post_type':
								...
								if ( $rule->applies_to( 'content_type', 'taxonomy' ) ) {
									...
									if ( ! empty( $rule_object_ids ) && is_array( $rule_object_ids ) ) {
										...
										if ( $rule->applies_to( 'content_type_name', 'product_cat' ) &&  'product_variation' === get_post_type( $object_id ) && in_array( 'purchasing_discount', $args['rule_type'], true ) ) {
