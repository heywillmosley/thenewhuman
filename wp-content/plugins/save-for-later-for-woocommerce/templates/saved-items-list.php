...
<div class="sa_saved_items_list_wrapper">
	<div class="sa_saved_items_list_container">
		<h2><?php echo __( 'Saved for later', SA_Save_For_Later::$text_domain ) . ' ' . sprintf(_n( '(%s item)', '(%s items)', count( $cart_items ), SA_Save_For_Later::$text_domain ), '<span class="sa_saved_item_count">' . count( $cart_items ) . '</span>' ); ?></h2>
			...
			<tbody>
				...
				?>
					...
					<tr>
						...
						<td class="product-name" data-title="<?php _e( 'Product', SA_Save_For_Later::$text_domain ); ?>">
							<?php
								if ( ! $_product->is_visible() ) {
								...
								if ( ! empty( $meta_data ) ) {
									echo '<p class="sa_product_meta">' . $meta_data . '</p>';
								}

								// Backorder notification
								if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
									echo '<p class="backorder_notification">' . esc_html__( 'Available on backorder', SA_Save_For_Later::$text_domain ) . '</p>';
								} else {
								...
								$saved_item_action_links = array(
																sprintf(
						                                            ...
						                                            __( 'Delete this item from Saved for later list', SA_Save_For_Later::$text_domain ),
						                                            esc_attr( $product_id ),
						                                            __( 'Delete', SA_Save_For_Later::$text_domain )
						                                        ),
																sprintf(
						                                            '<a href="%s" class="sa_move_to_cart" title="%s" data-product_id="%s">%s</a>',
						                                            esc_url( $sa_save_for_later->get_move_to_cart_url( $cart_item_key ) ),
						                                            __( 'Move this item to cart', SA_Save_For_Later::$text_domain ),
						                                            esc_attr( $product_id ),
						                                            __( 'Move to cart', SA_Save_For_Later::$text_domain )
						                                        )
															);

								$saved_item_action_links = apply_filters( 'saved_item_action_links', $saved_item_action_links, $_product );

								echo '<p class="sa_saved_item_actions">' . implode( ' | ', $saved_item_action_links ) . '</p>';

							?>

						</td>

						<td class="product-price" data-title="<?php _e( 'Price', SA_Save_For_Later::$text_domain ); ?>">
							<?php
