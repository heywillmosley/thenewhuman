...
<?php _e('This is an OPTIONAL settings page (and it is not required for your plugin to work).  The E-Signature Advanced Email settings will only affect the emails that are sent from WP E-Signature and will NOT affect your overall WordPress site email settings.  Sending from an SMTP is a secondary option for sending signer invite emails if you are currently experiencing issues sending emails.  <a href="https://wordpress.org/plugins/wpmandrill/" target="_blank">WP Mandrill</a> is another (free) third-party option (which we recommend exploring). ', 'esig'); ?> <a href='https://www.approveme.me/wp-digital-signature-plugin-docs/article/wordpress-smtp-plugin-settings/' target='_blank'><?php _e('Here\'s a Helpful Troubleshooting Article', 'esig'); ?></a>
        </div>
