...
function esig_addRecipient(){
	...
	for($i=0; $i < count($_POST['recipient_emails']); $i++){
		...
		$content .= '<p>
					<input type="text" name="recipient_fnames_ajax[]" placeholder="Signers Name" value="'. esc_html(stripslashes($fname[$i])) .'" readonly />
					<input type="text" name="recipient_emails_ajax[]" placeholder="Signers Email" value="'. $_POST['recipient_emails'][$i] .'" readonly /><a href="#" id="standard_view">'.__('Edit','esign').'</a>';
		$content .= '</p>'; 
...
function esig_auto_save_ajax(){
	...
	else
	{
		...
		if(!empty($esig_type) && $esig_type !="normal")
		{
			 ...
			 $wpdb->update($documents_table, 
				array('document_type'=>$esig_type),
			...
			if(isset($esign_auto_update) && !empty($esign_auto_update) )
			{
				...
				foreach($plugin_list as $plugin)
				{
					   if(array_key_exists($plugin->addon_id,$auto_downloads))
					   {
						  ...
						  // for single installation continue loop_tail
						  if($install_now != 'now')
							{
								break;
							}
