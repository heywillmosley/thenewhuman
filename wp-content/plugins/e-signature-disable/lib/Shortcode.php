...
class WP_E_Shortcode {
        ...
        if (!isset($_POST['recipient_signature']) && empty($_POST['recipient_signature']) && !isset($_POST['esignature_in_text']) && empty($_POST['esignature_in_text'])) {
            ...
            if ($this->doc_signature_validates()) {
                ...
                if (!empty($_POST['recipient_first_name'])) {
                    $f_name = $this->validation->esig_valid_string($_POST['recipient_first_name']);
                } else {
                    $f_name = "";
                }
                if (!empty($_POST['recipient_last_name'])) {
                    $l_name = $this->validation->esig_valid_string($_POST['recipient_last_name']);
                } else {
                    $l_name = "";
                }
                ...
                if ($f_name != $user_name) {
                    ...
                    // saving event
                    $event_text = sprintf(__("Signer name %s was changed to %s by %s IP: %s", 'esig'), $user_name, $f_name, $recipient->user_email, $_SERVER['REMOTE_ADDR']);
                    $this->document->recordEvent($doc_id, 'name_changed', $event_text, null);
                }

                $event_text = sprintf(__("Document signed by %s - %s IP %s", 'esig'), $f_name, $recipient->user_email, $_SERVER['REMOTE_ADDR']);
    ...
    public function notify_owner($document, $recipient, $audit_hash, $attachments = false) {
        ...
        $subject = sprintf(__("%s - Signed by %s %s","esign"),$document->document_title,$recipient->first_name,$recipient->user_email);
    ...
    public function notify_signer($document, $recipient, $post, $audit_hash, $attachments = false) {
        ...
        //R$subject = "{$document->document_title} has been signed";
    ...
    public function auditReport($id, &$document_data = null, $return_type=false){
        ...
        if($return_type) {
            ...
            $audit_trail_data->audit_signature_id =  $this->document->getSignedresult($id) ? wp_hash($audittrail->html) : false;
