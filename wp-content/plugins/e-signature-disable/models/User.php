...
public function wp_user_not_exists($email=null){
	...
	public function getUserBy($field, $strvalue){
        ...
        public function esig_is_signer($user_id)
        {
			...
			$this->wpdb->prepare(
					"SELECT is_signer FROM " . $this->table . " WHERE user_id=%d", 
					$user_id
					)
				);
              if($is_signer == 1) return true ;
	...
	public function getUserdetails($user_id, $document_id){
		...
		if(!empty($user[0])) {
                        ...
                        if($this->settings->get_generic("esign_user_meta_id_". $user_id ."_name_document_id_".$document_id)){
                            ...
                            $user[0]->first_name =esc_html(stripslashes( $this->settings->get_generic("esign_user_meta_id_". $user_id ."_name_document_id_".$document_id))) ; 
				...
				if($this->settings->get_generic("esign_user_meta_email_". $user_id ."_name_document_id_".$document_id)){
					$user[0]->user_email = $this->settings->get_generic("esign_user_meta_email_". $user_id ."_name_document_id_".$document_id) ; 
                        ...
                        elseif($this->signer->get_document_signer_info($user_id,$document_id))
                        {
                              ...
                              $user[0]->first_name = esc_html(stripslashes( $signers->signer_name));
                        }
			else {
				$user[0]->first_name = esc_html(stripslashes( $user[0]->first_name)) . " " . esc_html(stripslashes($user[0]->last_name));
			}
			return $user[0];
		}
		else 
	...
	public function  get_esig_signer_name($user_id, $document_id)
	{
	    ...
	    if($new_name)
	    {
            ...
            elseif($this->signer->get_document_signer_info($user_id,$document_id))
            {
                    ...
                    $signers = $this->signer->get_document_signer_info($user_id,$document_id); 
                    return esc_html(stripslashes( $signers->signer_name));
            }
	    else
	    {
	    ...
	    return esc_html(stripslashes_deep($signer_name)) ; 
	}
	...
	public function insert($user){
		...
		$uuid = UUID::v4();
			...
			if(!empty($user['user_title'])) { $user_user_title=$user['user_title'] ; } else {$user_user_title='';}
                        if(!isset($user['is_admin'])) { $is_admin=0 ; } else { $is_admin = 1;}
                        if(!isset($user['is_signer'])) { $is_signer=0 ; } else { $is_signer = 1;}
                        if(!isset($user['is_sa'])) { $is_sa=0 ; } else { $is_sa = 1;}
                        if(!isset($user['is_inactive'])) { $is_inactive=0 ; } else { $is_inactive = 1;}
                        
		$this->wpdb->query(
			$this->wpdb->prepare(
				"INSERT INTO " . $this->table . " VALUES(null,%d,%s,%s,%s,%s,%s,%d,%d,%d,%d)",
				$user_wp_user_id,
				$uuid,
				$user['user_email'],
				$user_user_title,
