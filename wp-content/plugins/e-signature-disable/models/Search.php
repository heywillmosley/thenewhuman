...
class WP_E_Search extends WP_E_Model
{
	...
	public function __construct(){
		...
		$offset = ( $pagenum - 1 ) * $limit;
                ...
                {
                    /****
                        select wp_esign_documents.document_id,wp_esign_documents.document_title,wp_esign_documents.document_status 
                        from wp_esign_documents LEFT JOIN wp_esign_document_users ON wp_esign_documents.document_id = wp_esign_document_users.document_id 
                        where wp_esign_document_users.signer_name LIKE "%kaunia%" AND wp_esign_documents.document_status='awaiting' 
                        OR wp_esign_documents.document_status ='awaiting' 
                        and wp_esign_documents.document_title LIKE "%kaunia%" Limit 0,30
                     */
                      //$docs=$this->wpdb->get_results($this->wpdb->prepare("SELECT ". $this->table .".document_id,".$this->table .".document_title FROM " . $this->table . " LEFT JOIN ". $this->usertable ." ON ". $this->table .".document_id =". $this->usertable .".document_id "
                        //      . "WHERE document_status=%s and document_title LIKE %s or signer_name LIKE %s LIMIT %d,%d",$document_status,$search,$search,$offset,$limit));
                     ...
                     // $docs=$this->wpdb->get_results($this->wpdb->prepare("SELECT * FROM " . $this->table ." WHERE document_title LIKE %s LIMIT %d,%d",$search,$offset,$limit));
                      
                      //$docs = array_merge($docs,$docs_users);
                }
                else
                {
                    
                     $user_id = $this->get_search_user_id();
                   // echo $this->wpdb->prepare("SELECT * FROM " . $this->table . " INNER JOIN ". $this->usertable ." ON ". $this->table .".document_id =". $this->usertable .".document_id WHERE ". $this->table .".user_id=%d and ".$this->table.".document_title LIKE %s OR ".$this->table.".user_id=%d and ". $this->usertable .".signer_name LIKE %s LIMIT %d,%d",$user_id,$search,$user_id, $search,$offset,$limit);
                    //exit;
		...
		$document_status= isset( $_GET['document_status'] ) ?  $_GET['document_status'] : "awaiting";
                ...
                {
                    $user_id = $this->get_search_user_id();
                     $docs=$this->wpdb->get_results($this->wpdb->prepare("SELECT * FROM " . $this->table . " INNER JOIN ". $this->usertable ." ON ". $this->table .".document_id =". $this->usertable .".document_id WHERE ". $this->table .".user_id=%d and ".$this->table.".document_title LIKE %s OR ".$this->table.".user_id=%d and ". $this->usertable .".signer_name LIKE %s",$user_id,$search,$user_id, $search));
                }
	...
	*/
	public function pagination()
	{
		...
		$pagenum = isset( $_GET['pagenum'] ) ? absint( $_GET['pagenum'] ) : 1;
                ...
                $esig_document_search = isset($_GET['esig_document_search']) ? sanitize_text_field($_GET['esig_document_search']) : null;
