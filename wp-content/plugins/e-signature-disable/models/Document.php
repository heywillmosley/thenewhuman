...
class WP_E_Document extends WP_E_Model {
    ...
    public function insert($post) {
        ...
        // recording event 
        $event_text = sprintf(__("%s Uploaded by %s - %s IP %s", 'esig'), $document_title, $admin_name, $admin_email, $_SERVER['REMOTE_ADDR']);
    ...
    public function update($post) {
        ...
        $affected = $this->wpdb->query(
         ...
         $event_text = sprintf(__("%s Uploaded by %s - %s IP %s", 'esig'), $document->document_title, $admin_name, $admin_email, $_SERVER['REMOTE_ADDR']);
...
EOL;
    ...
    public function recordView($id, $user_id, $date = null) {
        ...
        $event_text = sprintf(__("Document viewed by %s - %s IP %s", 'esig'), $signer_name, $this->user->getUserEmail($user_id), $_SERVER['REMOTE_ADDR']);
    ...
    public function esig_event_exists($document_id, $event) {
        return $this->wpdb->query(
                        $this->wpdb->prepare(
                                "SELECT id FROM " . $this->eventsTable . " WHERE document_id=%d and event='%s'", $document_id, $event
                        )
        );
    }
