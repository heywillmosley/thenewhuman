...
class BadgeOS_Sensei {
	...
	public function maybe_disable_plugin() {
		...
		if ( !$this->meets_requirements() ) {
			...
			if ( !class_exists( 'BadgeOS' ) || !function_exists( 'badgeos_get_user_earned_achievement_types' ) || version_compare( BadgeOS::$version, '1.3.0', '<' ) ) {
				echo '<p>' . sprintf( __( 'BadgeOS Sensei Add-On requires BadgeOS 1.3.0+ and has been <a href="%s">deactivated</a>. Please install and activate BadgeOS and then reactivate this plugin. <a href="%s">Install BadgeOS</a>', 'badgeos-sensei' ), admin_url( 'plugins.php' ), admin_url( 'plugin-install.php?tab=search&amp;s=badgeos' ) ) . '</p>';
			}
