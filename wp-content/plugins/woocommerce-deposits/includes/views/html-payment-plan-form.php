	...
	<form id="deposit-plan-form" method="post">
		...
		<div class="form-field">
			...
			$interval_units = '
				...
				';
			$row = '<tr>
					<td class="cell-amount"><input type="number" placeholder="0" step="1" min="0" name="plan_amount[]" class="plan_amount" /></td>
			...
			<table class="wc-deposits-plan" cellspacing="0" data-row="<?php echo esc_attr( $row ); ?>">
				...
				<tbody>
					<?php foreach ( $payment_schedule as $schedule ) :
						...
						<tr>
							<td class="cell-amount"><input type="number" placeholder="0" step="1" min="0" name="plan_amount[]" class="plan_amount" value="<?php echo esc_attr( $schedule->amount ); ?>" /></td>
							<td class="cell-percent">%</td>
							<?php if ( 0 === $index ) : ?>
								<td colspan="3">
