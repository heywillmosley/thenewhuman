<div id="frm_usermeta_<?php echo $meta_name ?>" class="frm_usermeta_row">
	<label class="frm_left_label" style="width:60px;min-width:0;"><?php _e( 'Name', 'frmreg' ) ?></label>
	<input type="text" value="<?php echo esc_attr( ( isset( $echo ) && $echo ) ? $meta_name : '' ) ?>" name="options[reg_usermeta][meta_name][<?php echo esc_attr( $meta_name ) ?>]"/>

	<?php _e( 'Form Field', 'frmreg' ) ?>
	<select name="options[reg_usermeta][field_id][<?php echo esc_attr( $meta_name ) ?>]">
