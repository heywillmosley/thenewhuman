...
<p><?php printf( __( 'Hi, <br>Your Balance Payment for %s from payment #%s is currently Overdue. <br>Please make the payment using the payment link %s before <strong>%s</strong>. If Payment is not received within <strong>%s</strong>, the order will be Cancelled.' , _sumo_pp()->text_domain ) , $product_title , _sumo_pp_get_payment_number( $payment_id ) , '<a href="' . esc_url( $order->order->get_checkout_payment_url() ) . '">' . __( 'pay' , _sumo_pp()->text_domain ) . '</a>' , $overdue_date , $overdue_date ) ; ?></p>
