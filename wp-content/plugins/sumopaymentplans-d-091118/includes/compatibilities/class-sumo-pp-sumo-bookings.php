...
class SUMO_PP_SUMOBookings {
    ...
    public static function get_booking_data_from_session( $product_id = null ) {
        $booking_data = WC()->session->get( _sumo_pp()->prefix . 'sumo_booking_data' ) ;
    ...
    public static function maybe_clear_booking_session( $product_id ) {
        ...
        WC()->session->set( _sumo_pp()->prefix . 'sumo_booking_data' , $booking_cost_from_session ) ;
    }
    ...
    public static function remove_default_payment_fields() {
        ...
        if ( isset( $post->ID ) ) {
            ...
            $product_props = SUMO_PP_Frontend::get_product_props( $post->ID ) ;

            if ( self::can_add_booking_cost( $product_props ) ) {
                remove_action( 'woocommerce_before_add_to_cart_button' , 'SUMO_PP_Frontend::add_simple_payment_type_fields' , 10 ) ;
            }
        }
    }
    ...
    public static function add_payment_fields() {
        ...
        if ( isset( $post->ID ) ) {
            ...
            if ( self::can_add_booking_cost( $product_props ) ) {
                echo '<span id="' . _sumo_pp()->prefix . 'wc_booking_deposit_fields"></span>' ;
                echo SUMO_PP_Frontend::get_payment_type_fields( null , false , _sumo_pp()->prefix . 'wc_booking_deposit_fields' ) ;
            }
        }
    }
    ...
    public static function get_booking_data( $booking_cost , $product_id ) {
        ...
        if ( is_numeric( $product_id ) && $product_id ) {
            $product_props = SUMO_PP_Frontend::get_product_props( $product_id ) ;
            ...
            if ( self::can_add_booking_cost( $product_props ) && is_numeric( $booking_cost ) ) {
                ...
                WC()->session->set( _sumo_pp()->prefix . 'sumo_booking_data' , array (
    ...
    public static function set_booking_cost( $product_props ) {
        ...
        if ( self::can_add_booking_cost( $product_props ) ) {
            if ( 'before' === $product_props[ 'pay_balance_type' ] ) {
                $end_payment_before_days = absint( get_post_meta( $product_props[ 'product_id' ] , _sumo_pp()->prefix . 'pay_balance_before_booked_date' , true ) ) ;
    ...
    public static function payment_is_cancelled( $payment_id , $order_id , $order_type ) {
        ...
        if ( 'pay-in-deposit' === get_post_meta( $payment_id , '_payment_type' , true ) && 'before' === get_post_meta( $payment_id , '_pay_balance_type' , true ) ) {
