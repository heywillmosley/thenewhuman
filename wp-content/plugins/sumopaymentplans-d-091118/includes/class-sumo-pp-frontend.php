...
class SUMO_PP_Frontend {
    ...
    public static function get_product_props( $product , $user_id = null ) {
        ...
        if ( isset( $user->roles[ 0 ] ) && in_array( $user->roles[ 0 ] , ( array ) get_option( _sumo_pp()->prefix . 'hide_payment_plans_only_for' ) ) ) {
                ...
                <div class="<?php echo $class ; ?>" id="<?php echo _sumo_pp()->prefix . 'payment_type_fields' ; ?>" <?php echo $hide_if_variation ? 'style="display:none;"' : '' ; ?>>
                    ...
                    <div id="<?php echo _sumo_pp()->prefix . 'plans_to_choose' ; ?>" <?php echo 'yes' === self::$product_props[ 'force_deposit' ] ? '' : 'style="display: none;"' ; ?>>
                        ...
                        if ( is_array( self::$product_props[ 'selected_plans' ] ) ) {
                            foreach ( self::$product_props[ 'selected_plans' ] as $plan_id ) {
                                if ( ! $plan = get_post( $plan_id ) ) {
                                    continue ;
                                }
                                ?>
                                <p>
                                    <input type="radio" value="<?php echo absint( $plan_id ) ; ?>" name="<?php echo _sumo_pp()->prefix . 'chosen_payment_plan' ; ?>" <?php echo 1 === $i ? 'checked="checked"' : '' ?>/>
                                    ...
                                    <?php } ?>
                                    <br><?php echo get_post_meta( $plan_id , '_plan_description' , true ) ; ?>
                                </p>                                    
                                <?php
                                $i ++ ;
                            }
    ...
    public static function get_item_meta( $cart_item , $to_display = false ) {
        ...
        if ( 'payment-plans' === $cart_data[ 'payment_product_props' ][ 'payment_type' ] ) {
            ...
            $meta_data = array (
                ...
                'next_payment_date'  => $to_display ? _sumo_pp_get_date_to_display( isset( $scheduled_payments_date[ 0 ] ) ? $scheduled_payments_date[ 0 ] : 0  ) : (isset( $scheduled_payments_date[ 0 ] ) ? $scheduled_payments_date[ 0 ] : 0) ,
            ...
            $meta_data = array (
                ...
                'balance_payable'   => $to_display ? wc_price( $balance_payable_amount ) : $balance_payable_amount ,
                'next_payment_date' => $to_display ? _sumo_pp_get_date_to_display( $next_payment_date ) : $next_payment_date ,
    ...
    public static function force_guest_signup_on_checkout( $checkout ) {
        if ( ! is_user_logged_in() && is_checkout() && isset( $checkout->enable_signup ) && isset( $checkout->enable_guest_checkout ) && _sumo_pp_cart_has_payment_items() ) {
