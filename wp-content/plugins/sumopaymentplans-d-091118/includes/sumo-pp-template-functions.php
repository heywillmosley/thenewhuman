...
function _sumo_pp_get_payment_orders_table( $payment_id , $args = array () , $echo = true ) {
    ...
    $args                    = wp_parse_args( $args , array (
        ...
        'th_elements'    => array (
            ...
            'paid-order'            => __( 'Paid Order' , _sumo_pp()->text_domain ) ,
        ) ,
        'page'           => 'admin' ,
            ) ) ;
        ...
        <tbody>
            ...
            if ( 'pay-in-deposit' === $payment_type ) {
                ...
                <tr>
                        ...
                        <td>
                            ...
                            if ( $next_payment_date = get_post_meta( $payment_id , '_next_payment_date' , true ) ) {
                                ...
                                if ( 'before' === get_post_meta( $payment_id , '_pay_balance_type' , true ) ) {
                                    ...
                                    echo _sumo_pp_get_date_to_display( _sumo_pp_get_timestamp( "+{$pay_balance_after} days" , _sumo_pp_get_timestamp( $payment_start_date ) ) ) ;
                                }
                            }
                            ?>
                        </td>
                    ...
                    <?php } ?>
                    <?php if ( in_array( 'paid-order' , $column_keys ) ) { ?>
                        <td>
                            <?php
                            if ( $balance_paid_order > 0 ) {
                                printf( __( '<a href="%s">#%s</a>' ) , $url , $balance_paid_order ) ;
                            } else {
                                echo '--' ;
                            }
                            ?>
