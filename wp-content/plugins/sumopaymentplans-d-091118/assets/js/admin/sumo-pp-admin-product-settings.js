/* global sumo_pp_admin_product_settings */

jQuery( function ( $ ) {

    // sumo_pp_admin_product_settings is required to continue, ensure the object exists
    if ( typeof sumo_pp_admin_product_settings === 'undefined' ) {
        return false ;
    }
        ...
        validate_save_product : function ( variation_index ) {
            ...
            var mayBeVariation = 'variable' === $( '#product-type' ).val() ? variation_index : '' ,
                    ...
                    availableErrFields = [
                        ...
                        '#_sumo_pp_pay_balance_after' + mayBeVariation ,
                    ] ,
                    prevField ;
            ...
            if ( $( '#_sumo_pp_enable_sumopaymentplans' + mayBeVariation ).is( ':checked' ) && 'pay-in-deposit' === $( '#_sumo_pp_payment_type' + mayBeVariation ).val() && ! $( '#_sumo_pp_apply_global_settings' + mayBeVariation ).is( ':checked' ) ) {
                if ( 'pre-defined' === $( '#_sumo_pp_deposit_type' + mayBeVariation ).val() ) {
                    ...
                    if ( 'fixed-price' === $( '#_sumo_pp_deposit_price_type' + mayBeVariation ).val() ) {
                        if ( '' === mayBeVariation ) {
                            var $regularPrice = parseFloat( $( '#_regular_price' ).val().replace( sumo_pp_admin_product_settings.decimal_sep , '.' ) ) ,
                                    $salePrice = parseFloat( $( '#_sale_price' ).val().replace( sumo_pp_admin_product_settings.decimal_sep , '.' ) ) ;
                        } else {
                            $regularPrice = parseFloat( $( '#variable_regular_price_' + mayBeVariation ).val().replace( sumo_pp_admin_product_settings.decimal_sep , '.' ) ) ,
                                    $salePrice = parseFloat( $( '#variable_sale_price' + mayBeVariation ).val().replace( sumo_pp_admin_product_settings.decimal_sep , '.' ) ) ;
                        }

                        var $subscriptionPrice = isNaN( $salePrice ) ? $regularPrice : $salePrice ,
                                $depositPrice = parseFloat( $( '#_sumo_pp_fixed_deposit_price' + mayBeVariation ).val().replace( sumo_pp_admin_product_settings.decimal_sep , '.' ) ) ;
                ...
                if ( 'before' === $( '#_sumo_pp_pay_balance_type' + mayBeVariation ).val() ) {
                    ...
                    var $dueDays = parseInt( $( '#_sumo_pp_pay_balance_after' + mayBeVariation ).val() ) ;

                    if ( isNaN( $dueDays ) || 0 >= $dueDays ) {
                        if ( '' !== mayBeVariation ) {
                            $( '#_sumo_pp_pay_balance_after' + mayBeVariation ).closest( '.wc-metabox > .wc-metabox-content' ).show() ;
                        }

                        errFields.push( '#_sumo_pp_pay_balance_after' + mayBeVariation ) ;
                    }
                }
            }
    ...
    var _sumo_pp_simple_product = {
        ...
        get_payment_type : function ( $payment_type , $do_not_apply_gobal ) {
            ...
            $( '#_sumo_pp_selected_plans' ).closest( 'p' ).show() ;
            ...
            if ( 'pay-in-deposit' === $payment_type ) {
                ...
                $( '#_sumo_pp_selected_plans' ).closest( 'p' ).hide() ;
    ...
    var _sumo_pp_variation_product = {
        ...
        trigger_on_page_load : function ( i ) {
            ...
            $( '#_sumo_pp_selected_plans' + i ).select2() ;
        ...
        get_payment_type : function ( $payment_type , i , $do_not_apply_gobal ) {
            ...
            $( '#_sumo_pp_selected_plans' + i ).closest( 'p' ).show() ;
            ...
            if ( 'pay-in-deposit' === $payment_type ) {
                ...
                $( '#_sumo_pp_selected_plans' + i ).closest( 'p' ).hide() ;
